...
| `frontend/src/components/Layout.jsx` | Sidebar nav (Dashboard, Scenario Library, New Scenario, Settings, Admin — no duplicate "Build Scenario" quick action), classification banner, top bar with user/logout, admin-only token-usage meter |
...
| `frontend/src/pages/GameSession.jsx` | Map-focal gameplay: a full-bleed Operational Map (always mounted) plus a **collapsible right side panel** whose segmented switcher selects Moves / Turn Log / Intel / Scores. Single-row session header (title · status · `T#/#` · phase chip · Run AI / Adjudicate / Capitulate / Red Team / ⋮); thin turn-progress strip (H+ times in a hover tooltip); AI status surfaces in a slim strip that appears only while working. `activeTab` chooses the panel tab (`PANEL_TABS` = `TABS` minus `map`; `'map'` falls through to the Moves content); `panelOpen` toggles the panel, with a reopen handle on the map edge when collapsed. Note: `pickingDest`/maneuver destination is set only via the map right-click ("Move here"); the legacy `pickingDest` flow is never triggered. |
...
- **Typography convention**: UI chrome (headings, buttons, tab/section labels, table headers) uses the sans family (Inter, the Tailwind `font-sans` default — just omit `font-mono`); `font-mono` (IBM Plex Mono) is reserved for **data** — turn counts (`T#/#`), coordinates, grid refs, unit IDs, scores, timestamps, token counts. Use sentence case, not ALL-CAPS, for labels. (Layout, GameSession, Dashboard, and ScenarioLibrary follow this; several secondary screens — RedTeamConsole, MonteCarloAnalyzer, AARGenerator, OrdersForm, ScenarioBuilder, BriefingExport, Admin — still carry legacy `font-mono` labels pending a follow-up sweep.)
- **Game-session layout**: the map is the focal surface and is always mounted (preserves Leaflet zoom/pan); tabbed content lives in the collapsible side panel, not a top tab bar. When adding a gameplay view, add it to `PANEL_TABS`, not a new full-width tab.
